...
torch_X_train = torch_X_train.view(-1,28,28).float()
torch_X_test = torch_X_test.view(-1,28,28).float()
torch_X_valid = torch_X_valid.view(-1,28,28).float()
...
class EarlyStopping:
    ...
    def __call__(self, val_loss, model):
        if self.best_loss is None:
            ...
            self.best_loss = val_loss
            self.counter = 0
            torch.save(model.state_dict(), "./transformer_pytorch_model_weights")
            print("Model saved")
...
class TransformerClassifier(nn.Module):
    def __init__(self, input_dim=28, model_dim=64, num_heads=4, num_layers=2, num_classes=10):
        super(TransformerClassifier, self).__init__()
        self.embedding = nn.Linear(input_dim, model_dim)
        encoder_layer = nn.TransformerEncoderLayer(d_model=model_dim, nhead=num_heads, dim_feedforward=128, batch_first=True)
        self.transformer = nn.TransformerEncoder(encoder_layer, num_layers=num_layers)
        self.fc = nn.Linear(model_dim, num_classes)

    def forward(self, x):
        x = self.embedding(x)  # (batch, 28, 28) → (batch, 28, 64)
        x = self.transformer(x)  # (batch, 28, 64) → (batch, 28, 64)
...
#torch.save(model.state_dict(), "./transformer_pytorch_model_weights")
model.load_state_dict(torch.load("./transformer_pytorch_model_weights"))
